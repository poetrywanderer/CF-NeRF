...
class NeRF_KDE(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        ...
        self.mu_p_alpha = torch.nn.Parameter(torch.tensor([0.]))
        self.sigma_p_alpha = torch.nn.Parameter(torch.tensor([1.]))

        self.mu_p_rgb = torch.nn.Parameter(torch.tensor([0.]))
        self.sigma_p_rgb = torch.nn.Parameter(torch.tensor([1.]))

        self.mu_p_alpha_val = torch.nn.Parameter(torch.tensor([0.]))
        self.sigma_p_alpha_val = torch.tensor([10.])

        self.mu_p_rgb_val =  torch.nn.Parameter(torch.tensor([0.]))
        self.sigma_p_rgb_val = torch.tensor([10.])

        # self.mu_rgb = torch.tensor([0.])
        # self.sigma_rgb = torch.tensor([2.])
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            ...
            # rgb_var = torch.clamp(rgb_var, max=50)
...
class NeRF_KDE_2(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        """
        """
        super(NeRF_KDE_2, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.input_ch_views = input_ch_views
        ...
        if use_viewdirs:
            self.feature_linear = nn.Linear(W, W)
            self.alpha_linear = nn.Linear(W, 1)
            self.alpha_std_linear = nn.Linear(W, 1)
            self.rgb_linear = nn.Linear(W//2, 3)
            self.rgb_std_linear = nn.Linear(W//2, 3)
            # self.z_mu = nn.Linear(self.W//2, self.z_size)
            # self.z_logvar = nn.Linear(self.W//2, self.z_size)
        else:
            self.output_linear = nn.Linear(W, output_ch)
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            alpha_mean = self.alpha_linear(h)
            alpha_std = F.softplus(self.alpha_std_linear(h)) + 1e-05
            # assert alpha_std.isinf().any() == False
            # assert alpha_std.isnan().any() == False
            # alpha_std = torch.clamp(alpha_std, max=50)
            feature = self.feature_linear(h)
            h = torch.cat([feature, input_views], -1)
            ...
            rgb_mean = self.rgb_linear(h)
            rgb_std = F.softplus(self.rgb_std_linear(h)) + 1e-05
            # rgb_std = torch.clamp(rgb_std, max=50)
            
            outputs = torch.cat([rgb_mean, rgb_std, alpha_mean, alpha_std], -1)
        else:
            outputs = self.output_linear(h)
...
# --------------------
# test separate Model-2 from TS2
# --------------------

# --------------------
# test composed Model-1 from TS1
# --------------------
