...
class MaskedLinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, device, diagonal_zeros=False, bias=True):
    ...
    def forward(self, x):
        mask_weight = self.mask * self.weight
        mask_weight = mask_weight.to(x.device)
        output = x.mm(mask_weight)

        if self.bias is not None:
            return output.add(self.bias.expand_as(output).to(x.device))
        else:
            return output
...
class MaskedConv2d(nn.Module):
    ...
    def __init__(self, in_features, out_features, device, size_kernel=(3, 3), diagonal_zeros=False, bias=True):
        super(MaskedConv2d, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.size_kernel = size_kernel
        self.diagonal_zeros = diagonal_zeros
        self.weight = Parameter(torch.FloatTensor(out_features, in_features, *self.size_kernel)).to(device)
        if bias:
            self.bias = Parameter(torch.FloatTensor(out_features)).to(device)
        else:
            self.register_parameter('bias', None)
