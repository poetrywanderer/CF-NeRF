...
from model.flow.layers import MaskedConv2d, MaskedLinear
...
class Planar(nn.Module):
    ...
    def forward(self, zk, u, w, b):
        ...
        log_det_jacobian = torch.log(torch.abs(1 + torch.bmm(psi, u_hat))+ 1e-10)
        assert log_det_jacobian.isnan().any() == False
        assert log_det_jacobian.isinf().any() == False
...
class Sylvester(nn.Module):
    ...
    def _forward(self, zk, r1, r2, q_ortho, b, sum_ldj=True):
        ...
        diag_j += 1.
        diag_j = diag_j.abs() + 1e-08
        assert diag_j.isinf().any() == False
        log_diag_j = torch.log(diag_j)
...
class TriangularSylvester(nn.Module):
    ...
    def _forward(self, zk, r1, r2, b, permute_z=None, sum_ldj=True, is_test=False):
        """
        ...
        diag_j = self.der_h(r2qzb).squeeze(1) * diag_j
        diag_j += 1.
        # print('forward flow single loop inside compute entropy time00:',time.time()-t0)
        diag_j = diag_j.abs() + 1e-08
        # print('forward flow single loop inside compute entropy time01:',time.time()-t0)
        # assert diag_j.isinf().any() == False
        # print('forward flow single loop inside compute entropy time02:',time.time()-t0)
        log_diag_j = torch.log(diag_j)
        ...
        # print('forward flow single loop inside compute entropy time:',time.time()-t0)

        return z, log_det_j

    def forward(self, zk, r1, r2, q_ortho, b, sum_ldj=True, is_test=False):
        
        t0 = time.time()
        z, log_det_j = self._forward(zk, r1, r2, q_ortho, b, sum_ldj, is_test)
        # print('forward flow single loop inside time:',time.time()-t0)
